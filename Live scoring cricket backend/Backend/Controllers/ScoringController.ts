...
const wideDeliveryController = async (req: Request, res: Response) => {
    try {
      const matchId = req.params.matchId;
      const bowlerId = req.params.bowlerId; 
      const deliveryData = req.body;
       
      _matchSchema.deliveries({ 
                                                          })
  
      const match = await _matchSchema.findById(matchId);
      const bowler = _matchSchema.Team.players.currentBowlerfind((p:any) => p.playerId === bowlerId);
  

      const handleWideWithRuns = (delivery: any, match: any, bowler: any) => {
        _matchSchema.currentTeam.total_runs += delivery.runs;
        bowler.runs_given += delivery.runs;
      };
  
      handleWideWithRuns(deliveryData, match, bowler);
  
       await _matchSchema.save();
  
      res.status(200).send({ message: "Wide delivery processed successfully" });
    } catch (error) {
      res.status(500).send({ message: "Error processing wide delivery" });
    }
  };
